...
class Cifar10Folder(data.Dataset):
    def __init__(self, root, filename='cifar10_label',
                 base_size=32, transform=None, target_transform=None):
    ...
    def __getitem__(self, index):
        ...
        #print("lable_vector: {0}".format(label_vector))
        imgs_list = get_imgs(image, self.imsize,
                             transform=self.transform,
                             normalize=self.norm)
        #print("images:{0}".format(imgs_list[0].shape))
        return imgs_list, label, torch.FloatTensor(label_vector)

    def __len__(self):
        return len(self.imgs)

class ImageFolder(data.Dataset):
    def __init__(self, root, split_dir='image_train', custom_classes=None,
...
class TextDataset(data.Dataset):
    def __init__(self, data_dir, split='image_train', embedding_type='cnn-rnn',
                 base_size=64, transform=None, target_transform=None):
        self.transform = transform
        self.norm = transforms.Compose([
            transforms.ToTensor(),
